...
use edgedb_errors::{ClientConnectionEosError, ClientConnectionError};
use edgedb_errors::{Error, ErrorKind};
use edgedb_errors::{ProtocolEncodingError, ProtocolOutOfOrderError};
...
impl<'r> Reader<'r> {
    ...
    fn poll_message(&mut self, cx: &mut Context) -> Poll<Result<ServerMessage, Error>> {
        let Reader {
            ref mut buf,
            ref mut stream,
            ..
        ...
        let frame_len = loop {
            ...
            if buf_len > 5 {
                ...
                next_read = max(min(len + 1 - buf_len, MAX_BUFFER), BUFFER_SIZE);
            ...
            unsafe {
                ...
                match Pin::new(&mut *stream).poll_read(cx, dest) {
                    Poll::Ready(Ok(0)) => {
                        return Poll::Ready(Err(ClientConnectionEosError::build()));
                    }
                    Poll::Ready(Ok(bytes)) => {
                    ...
                    Poll::Ready(Err(e)) => {
                        return Poll::Ready(Err(ClientConnectionError::with_source(e)));
                    }
        ...
        let result = ServerMessage::decode(&mut Input::new(self.proto.clone(), frame))
...
impl<T: QueryResult> Stream for QueryResponse<'_, T> {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Option<Self::Item>> {
        ...
        while buffer.len() == 0 {
            match seq.reader.poll_message(cx) {
                Poll::Ready(Ok(ServerMessage::Data(data))) if error.is_none() => {
                    if *complete {
                        return Poll::Ready(Some(Err(ProtocolOutOfOrderError::with_message(
                            format!("unsolicited packet: {}", PartialDebug(data)),
                        ))));
                    }
                    buffer.extend(data.data.into_iter().rev());
                }
                Poll::Ready(Ok(m @ ServerMessage::CommandComplete(_))) if error.is_none() => {
                    if *complete {
                        return Poll::Ready(Some(Err(ProtocolOutOfOrderError::with_message(
                            format!("unsolicited packet: {}", PartialDebug(m)),
                        ))));
                    }
                    *complete = true;
                }
                Poll::Ready(Ok(ServerMessage::ReadyForCommand(r))) => {
                    if let Some(error) = error.take() {
                        ...
                        if !*complete {
                            ...
                            return Poll::Ready(Some(Err(ProtocolOutOfOrderError::with_message(
                                format!("unsolicited packet: {}", PartialDebug(pkt)),
                            ))));
                        }
                        seq.reader.consume_ready(r);
                        seq.end_clean();
                ...
                Poll::Ready(Ok(message)) => {
                    return Poll::Ready(Some(Err(ProtocolOutOfOrderError::with_message(format!(
                        "unsolicited packet: {}",
                        PartialDebug(message)
                    )))));
                }
                Poll::Ready(Err(e)) => {
        ...
        Poll::Ready(Some(
            T::decode(state, &chunk).map_err(ProtocolEncodingError::with_source),
        ))
    }
}
