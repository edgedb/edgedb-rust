...
// We intentially don't pass these tests. libpq's output puts the colon into the
// hostname and then fails to resolve it
...
test_case!(
    dsn_only_cloudsql_unix_and_tcp,
    "postgres:///db?host=127.0.0.1:5432,/cloudsql/project:region:instance-name,localhost:5433&user=spam",
    error="",
