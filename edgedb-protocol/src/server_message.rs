...
use crate::descriptors::{Descriptor, InputTypedesc, OutputTypedesc, TypePos};
use crate::encoding::{Decode, Encode, Headers, Input, Output};
...
impl CommandDataDescription {
    pub fn output(&self) -> Result<OutputTypedesc, DecodeError> {
        let ref mut cur = Input::new(self.proto.clone(), self.output_typedesc.clone());
        OutputTypedesc::decode_with_id(self.output_typedesc_id.clone(), cur)
    }
    pub fn input(&self) -> Result<InputTypedesc, DecodeError> {
        let ref mut cur = Input::new(self.proto.clone(), self.input_typedesc.clone());
