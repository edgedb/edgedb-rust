...
use crate::transaction::{transaction, transaction_no_retry, EdgeDBErrorRef, Transaction};
...
impl Client {
    ...
    /// }
    ///
    /// impl edgedb_tokio::EdgeDBErrorRef for MyError {
    ///     fn as_edgedb_error_ref(&self) -> Option<&edgedb_tokio::Error> {
    ///         match self {
    ...
    ///
    /// let conn = edgedb_tokio::create_client().await?;
    /// let val = conn.transaction_with_err::<i64, _, _, MyError>(|mut tx| async move {
    ///     let result = tx.query_required_single::<i64, _>("
    ///         WITH C := UPDATE Counter SET { value := .value + 1}
    ///         SELECT C.value LIMIT 1
    ///     ", &()
    ///     ).await?;
    ...
    /// as long as this error `E` implements `EdgeDBErrorRef` and `From<edgedb_tokio::Error>`.
    ///
    /// Transaction body must be encompassed in the closure. The closure **is only executed once**.
    /// This allows for closures to consume captured values.
    ///
    /// # Panics
    ///
    /// Function panics when transaction object passed to the closure is not
    ...
    ///
    /// let conn = edgedb_tokio::create_client().await?;
    /// let val = conn.transaction_no_retry::<i64, _, _, MyError>(|mut tx| async move {
    ///     let result = tx.query_required_single::<i64, _>("
    ///         WITH C := UPDATE Counter SET { value := .value + 1}
    ///         SELECT C.value LIMIT 1
    ///     ", &()
    ...
    /// # }
    /// ```
    pub async fn transaction_no_retry<T, B, F, E>(&self, body: B) -> Result<T, E>
    where
        B: FnOnce(Transaction) -> F,
        F: Future<Output = Result<T, E>>,
        E: From<Error> + EdgeDBErrorRef,
    {
        transaction_no_retry(&self.pool, &self.options, body).await
    }
