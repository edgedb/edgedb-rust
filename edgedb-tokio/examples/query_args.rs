...
async fn main() -> anyhow::Result<()> {
    ...
    let no_args: String = conn.query_required_single("select {'No args inside here'};", &()).await?;
    println!("No args:\n{no_args}");

    let arg = "Hi I'm arg";
    let one_arg: String = conn.query_required_single("select {'One arg inside here: ' ++ <str>$0};", &(arg,)).await?;
    ...
    let named_args_fail: Result<i32, _> = conn.query_required_single("select {<int32>$num1 + <int32>$num2}", &(10, 20)).await;
    assert!(format!("{named_args_fail:?}").contains("expected positional arguments, got num1 instead of 0"));
